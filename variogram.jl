...
function sample_loss(model_factory, loss)
	model = model_factory()
    return Dict(
		:params=>Flux.params(model), 
		:loss=> loss(model(x_train[:,:,1:100]), y_train_oh[:,1:100])
	)
end

# ╔═╡ 30ae5840-ff23-418b-8084-70474f2909cc
typeof(sample_loss(mnistSimpleCNN7, Flux.Losses.mse)[:loss])
...
function loss_sample(samples)
    df = DataFrame(params=Zygote.Params[], loss=Float32[])
	for _ in range(0, length=samples)
        push!(df, sample_loss(mnistSimpleCNN7, Flux.Losses.mse))
	end
	return df
end
...
# ╔═╡ 628a8f7f-936a-468a-8af7-6cec794c55de
sqrt(sum(
	x->x^2, 
	Iterators.flatten(few_evals[1,:params]) .-Iterators.flatten(few_evals[2,:params])
))

# ╔═╡ 68280143-6e88-4a2d-a8df-d514f3bf5d41
map(Iterators.product(few_evals[!,:params], few_evals[!,:params])) do (p1, p2)
    sqrt(sum(x->x^2, Iterators.flatten(p1) .- Iterators.flatten(p2)))
end

# ╔═╡ a49766d6-6da8-4eee-b284-24ab08ee140a
map(Iterators.product([1,2,3], [1,2,3])) do (p1, p2)
	return p1, p2
end
